// The class is used to find a failed test case having the minimum distance
...
public class e_FindMinDistanceFailtestcase {
	public static void main() throws Exception {
		...
		for (Entry entry : set) {
			...
			if (entryvalue < mindistance) {
				mindistanceindex = (Integer) entry.getKey();
				mindistance = entryvalue;
			}
		...
		for (Entry entry : set) {
			double entryvalue = (Double) entry.getValue();
			if (entryvalue == mindistance) {
				mindistancelist.add((Integer) entry.getKey());
			}
		}
		System.out.println(mindistanceindex + ":" + mindistance);
		System.out.println(mindistancelist);
	}
}
