// The class is used to find a failed test case having the maximum coverage
...
public class FindMaxCoverageFailtestcase {
	// 找出最大覆盖的失败测试用例
	public static void main() throws Exception {
		...
		// 3.寻找最大覆盖list
		int maxcoverIndex = Integer.MIN_VALUE;
		int maxcoverLength = Integer.MIN_VALUE;
		List<Integer> maxcoverlist = new ArrayList<Integer>();
		// 存放最大覆盖的list
		// 1.只有一个最大覆盖
		// 2.如果有多个最大覆盖
		for (Entry<Integer, Integer> es : hm2.entrySet()) {
			if (es.getValue() > maxcoverLength) {
				maxcoverLength = es.getValue();
				maxcoverIndex = es.getKey();
			}
		}
		// 输出所有的最大覆盖的失败测试用例的Index
		for (Entry<Integer, Integer> es : hm2.entrySet()) {
			if (es.getValue() == maxcoverLength) {
				System.out.println(es.getKey());
				maxcoverlist.add(es.getKey());
			}
		}

		for (int i : maxcoverlist) {
